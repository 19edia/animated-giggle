...
BANNER_JUMP = "fuga.png"
BANNER_VILLAGE = "vila.png"
BANNER_SLIME = "slime.png"
BANNER_LADY = "velhinha.png"
BANNER_INN = "estalagem.png"
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.message:
        ...
        await update.callback_query.message.edit_media(
            media=InputMediaPhoto(media=open(BANNER_MENU, 'rb'), caption=caption, parse_mode="HTML"),
...
async def class_button(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data in CLASSES:
        ...
        # Cria uma tabela (grid) para escolha de gênero
        keyboard = [
            [InlineKeyboardButton(GENDERS[0], callback_data=f"gender_{GENDERS[0]}"), InlineKeyboardButton(GENDERS[1], callback_data=f"gender_{GENDERS[1]}")],
            [InlineKeyboardButton(GENDERS[2], callback_data=f"gender_{GENDERS[2]}")]
        ]
        ...
        await jump_window(update, context)

    elif data == "see_village":
        await see_village(update, context)

    elif data == "slime_fight":
        await fight_slime(update, context)

    elif data == "meet_old_lady":
        await meet_old_lady(update, context)

    elif data == "enter_inn":
        await enter_inn(update, context)

    elif data == "get_food":
        await get_food(update, context)
...
async def start_chapter_1(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.message.edit_media(
        media=InputMediaPhoto(media=open(BANNER_CAP1, 'rb'), caption=text, parse_mode="HTML"),
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
...
async def magic_result(update: Update, context: ContextTypes.DEFAULT_TYPE, accepted: bool) -> None:
    ...
    keyboard = [[InlineKeyboardButton("🪟 Pular Janela", callback_data="jump_out")]]
...
async def jump_window(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    text = (
        "🪟 <b>A Fuga</b>\n\n"
        "Sem tempo a perder, você corre em direção à pequena janela da cela. "
        "Com um salto desesperado, você se joga para fora! O vento corta seu rosto antes de você atingir um monte de feno lá embaixo.\n\n"
        "Você está livre! Mas precisa se apressar."
    )
    keyboard = [[InlineKeyboardButton("🚶 Seguir Caminho", callback_data="see_village")]]
    await query.message.edit_media(
        media=InputMediaPhoto(media=open(BANNER_JUMP, 'rb'), caption=text, parse_mode="HTML"),
        reply_markup=InlineKeyboardMarkup(keyboard)
    )

async def see_village(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    text = (
        "🏘️ <b>Vila à Vista</b>\n\n"
        "Após caminhar por alguns minutos entre as árvores, você chega ao topo de uma colina. "
        "Ao longe, a fumaça das chaminés revela uma vila pacífica. Parece ser o lugar seguro que você procurava."
    )
    keyboard = [[InlineKeyboardButton("👣 Continuar", callback_data="slime_fight")]]
    await query.message.edit_media(
        media=InputMediaPhoto(media=open(BANNER_VILLAGE, 'rb'), caption=text, parse_mode="HTML"),
        reply_markup=InlineKeyboardMarkup(keyboard)
    )

async def fight_slime(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    text = (
        "🦠 <b>Encontro Inesperado</b>\n\n"
        "De repente, um Slime gelatinoso e verde pula do mato bloqueando seu caminho!\n\n"
        "Instintivamente, você se prepara. Com um movimento rápido, você golpeia a criatura, que se desfaz em uma poça inofensiva.\n\n"
        "O caminho agora está limpo para você finalmente entrar na vila."
    )
    keyboard = [[InlineKeyboardButton("🏰 Entrar na Vila", callback_data="meet_old_lady")]]
    await query.message.edit_media(
        media=InputMediaPhoto(media=open(BANNER_SLIME, 'rb'), caption=text, parse_mode="HTML"),
        reply_markup=InlineKeyboardMarkup(keyboard)
    )

async def meet_old_lady(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    text = (
        "👵 <b>Uma Oferta Generosa</b>\n\n"
        "Assim que você cruza os portões da vila, uma velhinha de olhar bondoso se aproxima:\n\n"
        "<i>— Olá, jovem viajante! Você parece exausto. Ofereço hospedagem segura por 3 dias em minha estalagem!</i>\n\n"
        "O Mago da guilda, que observava de longe, acena e entrega algumas moedas de ouro para a senhora, pagando sua estadia."
    )
    keyboard = [[InlineKeyboardButton("🚪 Entrar na Estalagem", callback_data="enter_inn")]]
    await query.message.edit_media(
        media=InputMediaPhoto(media=open(BANNER_LADY, 'rb'), caption=text, parse_mode="HTML"),
        reply_markup=InlineKeyboardMarkup(keyboard)
    )

async def enter_inn(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    text = (
        "🏡 <b>O Interior Aconchegante</b>\n\n"
        "Ao entrar, o calor da lareira te abraça. O lugar é extremamente aconchegante, com uma mesa de madeira farta e comida recém-preparada exalando um aroma delicioso.\n\n"
        "A velhinha sorri enquanto começa a organizar alguns mantimentos para você."
    )
    keyboard = [[InlineKeyboardButton("🥣 Aceitar Comida", callback_data="get_food")]]
    await query.message.edit_media(
        media=InputMediaPhoto(media=open(BANNER_INN, 'rb'), caption=text, parse_mode="HTML"),
        reply_markup=InlineKeyboardMarkup(keyboard)
    )

async def get_food(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    user_id = str(update.effective_user.id)
    players = load_players()
    p = players.get(user_id)

    # Adiciona o item ao inventário se ainda não existir
    novo_item = {"nome": "Ensopado aconchegante", "efeito": "+50 HP"}
    if "inventory" not in p:
        p["inventory"] = []
    
    if novo_item not in p["inventory"]:
        p["inventory"].append(novo_item)
        save_player(user_id, p)

    text = (
        "🎒 <b>Novo Item Adicionado!</b>\n\n"
        "A velhinha coloca um pote fumegante em sua bolsa:\n\n"
        "<i>— Coma isto quando estiver ferido, meu jovem. Vai te revigorar instantaneamente!</i>\n\n"
        "✨ Você recebeu: <b>Ensopado aconchegante</b> (+50 HP)"
    )
